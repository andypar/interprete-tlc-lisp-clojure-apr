...
(defn controlar-aridad
  "Si la longitud de una lista dada es la esperada, devuelve esa longitud. Si no, devuelve una lista con un mensaje de error (una lista con *error* como primer elemento)."
  [lista x]
  (if (== (count lista) x)
    x
    (if (> (count lista) x) '(*error* too-many-args) '(*error* too-few-args))))
...
(defn igual?
  ...
  [x y]
  (let [x1 (if (= x 'NIL) nil (if (= x ()) nil x))
        y1 (if (= y 'NIL) nil (if (= y ()) nil y))]
    (if (= x1 y1)
      true
      (if (every? some? (list x1 y1))
        (if (or (string? x1) (string? y1))
          (= x1 y1)
          (= (if (some? x1) (clojure.string/upper-case x1) nil)
             (if (some? y1) (clojure.string/upper-case y1) nil)))
        false))))
...
(defn error?
  ...
  (if (and (list? x) (not (empty? x)))
    (= 0 (compare (clojure.string/upper-case (first x)) "*ERROR*"))
    false))
...
(defn revisar-fnc
  ...
  [x]
  (if (and (list? x) (not (empty? x)))
    (if (= 0 (compare (clojure.string/upper-case (first x)) "*ERROR*")) x nil)))
...
; (b 7)
(defn actualizar-amb
  "Devuelve un ambiente actualizado con una clave (nombre de la variable o funcion) y su valor. Si el valor es un error, el ambiente no se modifica. De lo contrario, se le carga o reemplaza el valor."  
  [x y z]
  (if (error? z)
    x
    (if (>= (.indexOf x (symbol (.toLowerCase (str y)))) 0)
      (reverse (into ()
                     (assoc (into [] x)
                       (+ 1 (.indexOf x (symbol (.toLowerCase (str y))))) z)))
      (reverse (conj (reverse x) (symbol (.toLowerCase (str y))) z)))))
...
(defn buscar
  "Busca una clave en un ambiente (una lista con claves en las posiciones impares [1, 3, 5...] y valores en las pares [2, 4, 6...] y devuelve el valor asociado. Devuelve un mensaje de error si no la encuentra."  
  [x y]
  (if (< (.indexOf (map first (partition 2 y)) x) 0)
    (reverse (conj (reverse '(*error* unbound-symbol)) x))
    (nth (map second (partition 2 y))
         (.indexOf (map first (partition 2 y)) x))))
...
(defn fnc-append
  ...
  [x]
  (if (error? (controlar-aridad x 2))
    (controlar-aridad x 2)
    (let [pri (first x)
          sec (second x)
          x1 (if (= pri 'NIL) () (if (= pri nil) () pri))
          y1 (if (= sec 'NIL) () (if (= sec nil) () sec))]
      (if (and (seq? x1) (seq? y1))
        (if (empty? (concat x1 y1)) nil (concat x1 y1))
        (list '*error* 'list 'expected y1)))))
...
"Devuelve la fusion de los ambientes global y local."
  [x y z]
   (if (empty? x) (concat y z) '(*error* too-many-args)))
...
(defn fnc-equal
  ...
  [x]
  (cond (= (count x) 2) (if (true? (igual? (last x) (first x))) 't nil)
        (> (count x) 2) '(*error* too-many-args)
...
(defn fnc-read
  ...
  [x]
  (if (error? (controlar-aridad x 0))
    '(*error* not-implemented)
    (let [x (read)] (if (and (seq? x) (empty? x)) nil x))))
...
[x]
	(if (error? (controlar-aridad x 0)) '(*error* not-implemented) (newline)))
...
(defn fnc-add
 "Suma los elementos de una lista. Minimo 2 elementos." 
  [x]
   (cond (>= (count x) 2) (if (true? (every? false? (map symbol? x)))
                           (reduce + x)
                           (concat '(*error* number-expected)
                                   (filter symbol? x)))
        (< (count x) 2) '(*error* too-few-args)))
...
(defn fnc-sub
  ...
  [x]
  (cond (= (count x) 1) (if (false? (symbol? x))
                          (- 0 (reduce read-string x))
                          (concat '(*error* number-expected) x))
        (> (count x) 1) (if (true? (every? false? (map symbol? x)))
                          (reduce - x)
                          (concat '(*error* number-expected)
                                  (filter symbol? x)))
...
(defn fnc-lt
  ...
  [x]
  (if (true? (every? false? (map symbol? x)))
    (if (error? (controlar-aridad x 2))
      (controlar-aridad x 2)
      (if (true? (reduce < x)) 't nil))
    (concat '(*error* number-expected) (filter symbol? x))))
...
(defn fnc-reverse
 "Devuelve una lista con sus elementos en orden inverso."  
  [x]
  (if (true? (error? (controlar-aridad x 1)))
    (controlar-aridad x 1)
    (if (seq? (first x))
      (reverse (first x))
      (concat '(*error* list expected) x))))
...
(defn evaluar-escalar
  ...
  [x y z]
  (cond (not (symbol? x)) (list x y)
        true (let [x2 (symbol (clojure.string/lower-case x))]
               (if (true? (or (error? (buscar x2 y)) (error? (buscar x2 z))))
                 (if (true? (error? (buscar x2 y)))
                   (list (buscar x2 z) y)
                   (list (buscar x2 y) y))
                 (list (buscar x2 z) y)))))
...
(defn evaluar-de
  [x y]
  (cond (= 1 (count x)) (list '(*error* list expected nil) y)
        (= 2 (count x)) (list '(*error* list expected nil) y)
        (nil? (second x)) (list '(*error* cannot-set nil) y)
        (not (list? (nth x 2)))
          (if (= 3 (count x))
            (list (flatten (list '(*error* list expected) (last x))) y)
            (list (flatten (list '(*error* list expected) (second (rest x)))) y))
        (not (symbol? (second x)))
        (list (flatten (list '(*error* symbol expected) (second x))) y)
...
(defn evaluar-if
  ...
  [expre amb-global amb-local]
  (if (or (nil? (nnext expre))
          (and (nil? (second expre)) (= (count (next expre)) 2)))
    (list nil amb-global)
    (let [res-eval (evaluar (second expre) amb-global amb-local)]
      (if (not (igual? (first res-eval) nil))
        (if (error? (first res-eval))
          res-eval
          (evaluar (first (nnext expre)) amb-global amb-local))
        (if (or (nil? (first (next expre))) (igual? (first res-eval) nil))
          ;if (or (nil? (first (next expre))) (nil? (first res-eval)))
          (evaluar (last (nnext expre)) amb-global amb-local)
          (recur (cons 'if (nnext expre)) (second res-eval) amb-local))))))
...
(defn evaluar-or
  ...
  (if (nil? (next expre))
    ...
    (let [res-eval (evaluar (second expre) amb-global amb-local)]
      (if (not (igual? (first res-eval) nil))
        res-eval
        (recur (cons 'or (nnext expre)) (second res-eval) amb-local)))))
...
(defn evaluar-setq
  ...
  [expre amb-global amb-local]
  (let [kv (first (partition 2 (next expre)))
        es (evaluar (second kv) amb-global amb-local)
        res-eval (actualizar-amb amb-global (first kv) (first es))
        next-kv (second (partition 2 '2 '(nil) (next expre)))]
    (if (nil? (first kv))
      (if (nil? (second kv))
        (list '(*error* list expected nil) amb-global)
        (list (list '*error* 'cannot-set 'nil) amb-global))
      (if (not (symbol? (first kv)))
        (list (list '*error* 'symbol 'expected (first kv)) amb-global)
        (if (nil? next-kv)
          (list (first es) res-eval)
          (if (nil? (second next-kv))
            (list '(*error* list expected nil) res-eval)
            (evaluar-setq
              (concat (list 'setq
                            (first next-kv)
                            (first
                              (evaluar (second next-kv) res-eval amb-local)))
                      (flatten (nnext (partition 2 '2 '(nil) (next expre)))))
              res-eval
              amb-local)))))))
